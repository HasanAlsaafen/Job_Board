...
<div class="flex flex-col min-h-screen bg-white text-black" dir="{{ app()->isLocale('ar') ? 'rtl' : 'ltr' }}">
    ...
    <div class="max-w-4xl mx-auto w-full px-4 lg:px-8 py-6 pt-16 lg:pt-6">
        ...
        <div
            class="bg-white p-4 rounded-xl shadow-sm border border-blue-100 mb-6 flex flex-col md:flex-row gap-3 items-center">
            ...
            <select wire:model.live="selectedTag"
                class="w-full md:w-1/4 border border-blue-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        @if ($view === 'list')
            ...
            <div class="space-y-3">
                @foreach ($jobs as $job)
                    <div
                        class="bg-white px-6 py-5 rounded-xl shadow-sm border border-blue-100 hover:border-blue-300 hover:shadow-md transition duration-200">
                        <div class="flex justify-between items-start gap-3 mb-1">
                            ...
                            <span
                                class="shrink-0 px-2.5 py-1 text-xs font-semibold rounded-full bg-blue-50 text-blue-600 border border-blue-100">
                                {{ $job->type }}
                            </span>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-2 mt-3">
                            @if ($job->tags && $job->tags->isNotEmpty())
                        ...
                        <div
                            class="flex flex-wrap items-center justify-between pt-3 border-t border-blue-100 text-xs text-gray-400 gap-2">
                            ...
                            @auth
                                @if ($job->saved_by_users_exists)
                                    <button wire:click="unsaveJob({{ $job->id }})"
                                        ...
                                        <svg class="h-3.5 w-3.5 fill-current" xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 24 24">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <a href="{{ route('jobs.show', $job) }}"
                                    class="px-3 py-1.5 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition duration-150">
                    ...
                    jobs.forEach(job => {
                        L.marker([job.latitude, job.longitude])
                            ...
                            .bindPopup(`
                                                                                                            ...
                                                                                                            ${job.company_name}<br>
                                                                                                            ${job.location}<br>
                                                                                                            <a href='/jobs/${job.id}'>{{ __('messages.jobs_feed.view_details') }}</a>
                                                                                                        `);
                    });
                }
            }" x-init="initMap()"
