<div class="{{ app()->isLocale('ar') ? 'lg:mr-64' : 'lg:ml-64' }} flex flex-col min-h-screen bg-gray-50 text-black" dir="{{ app()->isLocale('ar') ? 'rtl' : 'ltr' }}">

    <div class="px-4 lg:px-8 pt-16 lg:pt-0">
        @include('livewire.partials.employer-sidebar', ['active' => 'applicants'])

        <header class="bg-white border-b border-gray-100 py-4 px-4 lg:px-6 mt-4 lg:mt-8 mb-6 rounded-xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                <a href="{{ route('employer.applicants') }}"
                   class="inline-flex items-center gap-1.5 text-xs text-gray-400 hover:text-blue-600 font-medium mb-1.5 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="size-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="{{ app()->isLocale('ar') ? 'M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3' : 'M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18' }}" />
                    </svg>
            ...
            <span class="shrink-0 text-sm font-bold text-blue-600 bg-blue-50 border border-blue-100 px-4 py-2 rounded-xl">
        ...
        @else
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 divide-y divide-gray-100 overflow-hidden">
                @foreach ($applications as $application)
                    <div class="p-5 sm:p-6 flex flex-col sm:flex-row sm:items-start justify-between gap-4" wire:key="application-{{ $application->id }}">

                        <div class="flex items-start gap-3 flex-1 min-w-0">
                            <div class="shrink-0 w-9 h-9 rounded-full bg-blue-100 text-blue-700 font-bold text-sm flex items-center justify-center uppercase">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if ($application->cover_letter)
                                    <div class="mt-2.5">
                                        ...
                                        <p class="text-xs text-gray-600 bg-gray-50 border border-gray-100 rounded-lg p-2.5 italic leading-relaxed line-clamp-3">
                        ...
                        <div class="flex sm:flex-col items-center sm:items-end gap-2 shrink-0">
                            <span class="inline-flex items-center text-xs font-semibold border px-2.5 py-1 rounded-lg {{ $statusStyles[$application->status] ?? 'bg-gray-50 text-gray-600 border-gray-200' }}">
