...
<div
    ...
    <div class="px-4 lg:px-8 pt-16 lg:pt-0">
        ...
        <header class="bg-white border border-brand-border rounded-lg py-4 px-5 mt-4 lg:mt-8 mb-6">
            <h1 class="font-display text-lg font-semibold text-brand-ink">{{ __('messages.employer_dashboard.title') }}</h1>
    ...
    <div class="flex flex-col px-4 lg:px-8 pb-8 space-y-6">
        ...
        <div class="bg-white border border-brand-border rounded-lg overflow-hidden max-w-2xl">
            <div class="px-5 py-3.5 border-b border-brand-border flex items-center justify-between">
                <h2 class="font-display text-sm font-semibold text-brand-ink">{{ __('messages.employer_dashboard.recent_applicants_title') }}</h2>
            ...
            <div class="divide-y divide-brand-border">
                ...
                @forelse ($recentApplicants as $applicant)
                    <div class="px-5 py-3 flex items-center justify-between gap-4">
                        <div class="min-w-0">
                            <p class="text-sm font-medium text-brand-ink truncate">{{ $applicant->user->name ?? '—' }}</p>
                            <p class="font-mono text-xs text-brand-muted truncate">{{ $applicant->jobListing->title ?? '—' }}</p>
                        </div>
                        <span
                ...
                @empty
                    <p class="px-5 py-6 font-mono text-xs text-brand-muted italic text-center">{{ __('messages.employer_dashboard.no_applicants') }}</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white border border-brand-border rounded-lg p-6 max-w-2xl">
            ...
            @if ($successMessage)
                <div class="flex items-center justify-between gap-3 p-3 mb-5 font-sans text-sm font-medium text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-md">
                    <div class="flex items-center gap-2">
                        <svg aria-hidden="true" class="h-4 w-4 shrink-0 text-emerald-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <form wire:submit.prevent="createJob" class="space-y-5 text-sm">
                ...
                <div>
                    <label for="job-title" class="block font-medium text-brand-ink mb-1.5">{{ __('messages.job_form.job_title') }}</label>
                ...
                <div>
                    <label for="job-company" class="block font-medium text-brand-ink mb-1.5">{{ __('messages.job_form.company_name') }}</label>
                ...
                <div>
                    <label for="job-location" class="block font-medium text-brand-ink mb-1.5">{{ __('messages.job_form.location') }}</label>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex flex-wrap gap-2 p-3 bg-brand-surface border border-brand-border rounded-md min-h-[48px]">
                        @php $availableTags = $tags->filter(fn($t) => !in_array($t->id, $selectedTagIds)); @endphp
                        @forelse ($availableTags as $tag)
                            <label class="cursor-pointer select-none">
                                <input type="checkbox" wire:model.live="selectedTagIds" value="{{ $tag->id }}" class="sr-only">
                                <span
                        ...
                        @empty
                            <span class="font-sans text-xs text-brand-muted self-center italic">{{ __('messages.job_form.all_tags_selected') }}</span>
                        @endforelse
                    </div>

                    @if (count($selectedTagIds) > 0)
                        <div class="p-3 border border-brand-primary/20 bg-brand-primary-light rounded-md">
                            <p class="font-sans text-xs text-brand-primary mb-2">{{ trans_choice('messages.job_form.tags_selected', count($selectedTagIds), ['count' => count($selectedTagIds)]) }}</p>
                                ...
                                @foreach ($tags->whereIn('id', $selectedTagIds) as $tag)
                                    <span
                                        ...
                                        <button type="button" wire:click="removeTag({{ $tag->id }})"
                                            ...
                                            <svg aria-hidden="true" class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                ...
                <div>
                    <label for="job-type" class="block font-medium text-brand-ink mb-1.5">{{ __('messages.job_form.employment_type') }}</label>
                ...
                <div>
                    <label for="job-salary" class="block font-medium text-brand-ink mb-1.5">{{ __('messages.job_form.salary_range') }}</label>
                ...
                <div>
                    <label for="job-expires" class="block font-medium text-brand-ink mb-1.5">{{ __('messages.job_form.expires_at') }}</label>
                ...
                <div>
                    <label for="job-description" class="block font-medium text-brand-ink mb-1.5">{{ __('messages.job_form.description') }}</label>
                ...
                <div>
                    <label for="job-requirements" class="block font-medium text-brand-ink mb-1.5">{{ __('messages.job_form.requirements') }}</label>
                ...
                <div class="border-t border-brand-border pt-5">
                    ...
                    }" x-init="initMap()" class="rounded-md overflow-hidden border border-brand-border">
                ...
                <button type="submit" wire:loading.attr="disabled"
                    ...
                    <svg wire:loading wire:target="createJob" aria-hidden="true" class="size-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
