            ...
            <div class="bg-white border border-brand-border rounded-lg overflow-hidden">
                ...
                <div class="flex justify-between items-center px-5 py-4 border-b border-brand-border">
                    <div>
                        ...
                        <span class="font-sans text-xs font-medium px-2 py-0.5 rounded bg-brand-surface-low border border-brand-border text-brand-muted mt-1 inline-block">
                            {{ $job->type }}
                        </span>
                    </div>
                    <span class="font-sans text-xs font-medium text-brand-primary bg-brand-primary-light border border-brand-primary/20 px-3 py-1.5 rounded">
        ...
        'Not proceeding'        => 'bg-brand-surface text-brand-muted border border-brand-border',
                ...
                <div class="divide-y divide-brand-border">
                    @foreach($job->applications as $application)
                        <div class="px-5 py-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                            <div class="flex-1 min-w-0">
                                <p class="font-semibold text-brand-ink text-sm">
                                    {{ $application->user->name }}
                                    <span class="font-mono text-xs font-normal text-brand-muted">({{ $application->user->email }})</span>
                                </p>
                                <p class="font-mono text-xs text-brand-muted mt-1.5 bg-brand-surface border border-brand-border rounded-md px-3 py-2 italic leading-relaxed">
